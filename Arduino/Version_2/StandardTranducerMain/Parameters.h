 ...
 * Parameters.h
 *
 * Created: 24/04/2018 12:22:39 p. m.
 *  Author: camil
 */ 

/*
* This file is used to declare the parameters table used by the program.
*
* We use the EEPROM for saving the parameters changed by user during the
* Standard Transducer operation.
*
* The parameter are loaded during the boot.
*
* All change to important parameters are saved to the EEPROM.
*/

#ifndef PARAMETERS_H_
#define PARAMETERS_H_

#include <avr/eeprom.h>    // Enable EEPROM memory use.  (_AVR_EEPROM_H_)
#ifndef TIME_H_
  #include "Time.h"
#endif
#ifndef ChNil_h
...
/*  PARAMETERS                                                          */
/************************************************************************/
#define MAX_PARAM 52   // If the MAX_PARAM change you need to change the 
             // pointer in the EEPROM.
...
#define EE_START_PARAM           0 // Save the parameter from byte 0 of EEPROM
...
int parameters[MAX_PARAM];      // Create an array for parameters.
...
boolean getStatus(byte statusFlag) {
  return getParameterBit(FLAG_STATUS, statusFlag);
}

boolean isEnabled(byte statusFlag) {
  return getParameterBit(FLAG_ENABLED, statusFlag);
}
...
boolean start(byte statusFlag) {
  boolean statusChanged = setParameterBit(FLAG_STATUS, statusFlag);
#ifdef EVENT_LOGGING
  if (statusChanged) {
...
boolean stop(byte statusFlag) {
  boolean statusChanged =  clearParameterBit(FLAG_STATUS, statusFlag);
#ifdef EVENT_LOGGING
  if (statusChanged) {
