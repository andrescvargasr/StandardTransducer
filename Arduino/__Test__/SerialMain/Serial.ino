  ...
  while (TRUE)
  {
    ...
    while (Serial.available()) {
      ...
      if (inChar == 13 || inChar == 10) { // CR & LF
        ...
        if (serialBufferPosition > 0) printResult(serialBuffer, &Serial);
      ...
      // Almacenar valor en la cadena y finalizar con \0.
        ...
        if (serialBufferPosition < SERIAL_BUFFER_LENGTH) {
          ...
          if (serialBufferPosition < SERIAL_BUFFER_LENGTH) serialBuffer[serialBufferPosition] = '\0';
  ...
  while (!theEnd) {
    ...
    // Verificar si es un número, incluyendo su signo.
        ...
        if (paramValuePosition < SERIAL_MAX_PARAM_VALUE_LENGTH) {
            ...
            paramValuePosition++;
            if (paramValuePosition < SERIAL_MAX_PARAM_VALUE_LENGTH) paramValue[paramValuePosition] = '\0';
        }
    }
    ...
    // Verificar si es una letra mayúscula.
        ...
        //// Se ha insertado un comando de variable
        //paramCurrent++;
        //if (paramCurrent > MAX_PARAM) paramCurrent = 0;
        //
        //if (paramValuePosition < SERIAL_MAX_PARAM_VALUE_LENGTH) {
            //paramValue[paramValuePosition] = inChar;
            //paramValuePosition++;
...
void printHelp(Print* output) {
  ...
  // Changed for (p)
  //output->println(F("s - (s)ettings"));
  output->println(F("p - (p)arameters"));
  // Imprimir utilidades, es la configuración en la pantalla OLED.
